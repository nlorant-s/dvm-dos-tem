...
===========================================
For interactive exploration of SA results (spaghetti plots, equilibrium checks, parameter–target relationships, nitrogen checks), use the Jupyter notebook [`notebooks/calibration_process.ipynb`](notebooks/calibration_process.ipynb). Run it inside the `dvmdostem-dev` or `dvmdostem-autocal` Docker container; set `WORK_DIR` in the notebook to the `work_dir` from your SA yaml (see [`agent_calibration/sa-step1-example-imn.yaml`](agent_calibration/sa-step1-example-imn.yaml) and [`agent_calibration_step2/sa-step2-example-imn.yaml`](agent_calibration_step2/sa-step2-example-imn.yaml) for CMT04 Imnavait examples). For automated calibration, see [`agent_calibration/`](agent_calibration/) (Step 1) and [`agent_calibration_step2/`](agent_calibration_step2/) (Step 2) — each README describes templates vs runtime paths. Agent-created SA configs go in `logs/` (gitignored); results are written under `/data/workflows/` via `--json-out`.
