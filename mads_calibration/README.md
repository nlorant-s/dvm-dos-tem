...
===========================================
For interactive exploration of SA results (spaghetti plots, equilibrium checks, parameter–target relationships, nitrogen checks), use the Jupyter notebook [`notebooks/calibration_process.ipynb`](notebooks/calibration_process.ipynb). Run it inside the `dvmdostem-dev` or `dvmdostem-autocal` Docker container; set `WORK_DIR` in the notebook to the `work_dir` from your SA yaml (see [`agent/agent_calibration_step1/sa-step1-example-imn.yaml`](agent/agent_calibration_step1/sa-step1-example-imn.yaml) and [`agent/agent_calibration_step2/sa-step2-example-imn.yaml`](agent/agent_calibration_step2/sa-step2-example-imn.yaml) for CMT04 Imnavait examples). For automated calibration, see [`agent/README.md`](agent/README.md) — Phase 0 setup through final evaluation under [`agent/`](agent/). Agent-created SA configs go in `logs/` (gitignored); results are written under `/data/workflows/` via `--json-out`.
